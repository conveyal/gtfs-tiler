...
function loadFeedsAndGenerateTiles () {
  ...
  const loadNextFeed = (i, done) => {
    ...
    let processedFeed
    // If cache is exists and is valid, process feed from JSON.
    if (fs.existsSync(filepath)) {
      console.log('- found, reading from cache..')
      try {
        const feedData = JSON.parse(fs.readFileSync(filepath))
        processedFeed = processFeed(feedData)
      } catch (e) {
        console.warn('cache corrupted... deleting and re-fetching feed.')
        // Delete cache synchronously.
        fs.unlinkSync(filepath)
      }
    }
    if (!processedFeed) {
      console.log('- not found (or invalid), querying GTFS API..')
      queryAndProcessFeed(feedIds[i])
        .then(processedFeed => {
...
function queryAndProcessFeed (feedId) {
...
function renderTile (tx, ty, zoom, patterns, stops) {
  ...
  if (fs.existsSync(filepath) && !config.render.force) return
